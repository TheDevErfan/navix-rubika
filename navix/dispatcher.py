...
class Dispatcher:
    ...
    async def start_polling(self, interval: float = 1.0):
        ...
        while self.is_running:
            try:
                ...
                # استفاده از متد request کلاینت که به API متصل است
    ...
    async def _process_update(self, update: dict):
        """
        عبور دادن آپدیت از میدلورها و سپس ارسال به هندلرهای روتر با بررسی فیلترها
        """
        msg = Message(update, client=self.client)

        for handler, filters in self.router.message_handlers:
            # بررسی فیلترهای هندلر
            passed = True
            for f in filters:
                if callable(f):
            ...
            # اجرای زنجیره میدلور برای هر هندلر تایید شده
